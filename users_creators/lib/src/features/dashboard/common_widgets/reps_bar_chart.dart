...
class RepsBarChards extends StatefulWidget {
  const RepsBarChards({
    Key? key,
    required this.reps,
  }) : super(key: key);
  final List<RepeatsDate> reps;

  @override
  _RepsBarChardsState createState() => _RepsBarChardsState();
}

class _RepsBarChardsState extends State<RepsBarChards> {
  void addingBarChars() {
    if (widget.reps.isNotEmpty) {
      for (int index = 0; index < widget.reps.length; index++) {
        chartData.add(ChartData(
          xAxis :  index == 0 || index == widget.reps.length - 1
              ? DateFormat.yMd().format(DateTime.parse(widget.reps[index].date!))
              : ' ',
          yAxis: widget.reps[index].repeats?.toDouble()
